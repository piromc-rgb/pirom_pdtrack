...
interface SettingsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onRefreshData: (newUrl?: string, newProdUrl?: string, newQcUrl?: string) => Promise<void>;
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleSaveAndSync = async () => {
    ...
    try {
      ...
      await onRefreshData(trimmed1, trimmed2, trimmed3);
      setSaveSuccess(true);
      setTimeout(() => {
        setSaveSuccess(false);
        onClose();
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="p-5 bg-slate-900 text-white flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base text-white">การเชื่อมต่อ Google Sheets (Dual-Sheet)</h3>
              <p className="text-xs text-slate-400">ผสาน Check list ส่งมอบ + Record รับ-จ่าย Production</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-5 sm:p-6 space-y-5">
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-xs space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className={`font-semibold flex items-center gap-1 ${isLive ? 'text-emerald-600' : 'text-amber-600'}`}>
                ...
                {isLive ? 'เชื่อมโยง 2 สเปรดชีตสดแบบอัตโนมัติ' : 'ใช้งานออฟไลน์/แคชสำรองที่เชื่อมโยงแล้ว'}
              </span>
            </div>
            <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-between pt-1">
            <span className="text-[11px] text-slate-500">
              * ระบบจะผสานข้อมูลส่งมอบ + ฝ่ายผลิต + สถานะ QC จากทั้ง 3 ไฟล์ให้อัตโนมัติ
            </span>
            <button
              onClick={handleResetDefault}
              className="text-xs text-slate-600 hover:text-slate-900 flex items-center gap-1 font-medium whitespace-nowrap ml-2"
            >
              <RotateCcw className="w-3 h-3" />
              <span>รีเซ็ตค่าเริ่มต้นทั้ง 3 ลิงก์</span>
            </button>
          </div>
          ...
          {saveSuccess && (
            <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-200 text-xs text-emerald-700 flex items-center gap-2">
              ...
              <span>เชื่อมต่อและผสานข้อมูลทั้ง 3 สเปรดชีตเรียบร้อยแล้ว!</span>
            </div>
          )}
        </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-end gap-2.5">
          ...
          <button
            ...
            disabled={isSaving}
            className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-sky-600 hover:bg-sky-700 active:scale-95 rounded-lg transition disabled:opacity-50"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isSaving ? 'animate-spin' : ''}`} />
            <span>{isSaving ? 'กำลังเชื่อมต่อทั้ง 2 สเปรดชีต...' : 'บันทึก & ซิงค์ข้อมูล'}</span>
          </button>
        </div>
