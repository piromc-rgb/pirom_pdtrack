...
export const SearchFilterBar: React.FC<SearchFilterBarProps> = ({
  ...
  const [isCollapsed, setIsCollapsed] = useState<boolean>(() => {
    return localStorage.getItem('pdtrack_filter_collapsed') === 'true';
  });
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm overflow-hidden space-y-0">
      ...
      <div 
        ...
        className={`px-5 py-3 bg-slate-50 ${!isCollapsed ? 'border-b border-slate-200' : ''} flex flex-wrap items-center justify-between gap-3 cursor-pointer select-none hover:bg-slate-100/80 transition`}
      >
        ...
        {/* Results indicator & Reset & Toggle Button */}
        <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
          ...
          {/* Icon Button to Hide / Unhide */}
          <button
            onClick={toggleCollapse}
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold transition shadow-2xs cursor-pointer ${
              isCollapsed
                ? 'bg-blue-600 hover:bg-blue-700 text-white border-blue-600'
                : 'bg-white hover:bg-slate-100 text-slate-700 border-slate-300'
            }`}
            title={isCollapsed ? "แสดงตัวกรอง (Unhide)" : "ซ่อนตัวกรอง (Hide)"}
          >
            {isCollapsed ? (
              <>
                <Eye className="w-3.5 h-3.5" />
                <span>แสดงตัวกรอง (Unhide)</span>
                <ChevronDown className="w-3.5 h-3.5" />
              </>
            ) : (
              <>
                <EyeOff className="w-3.5 h-3.5" />
                <span>ซ่อนตัวกรอง (Hide)</span>
      ...
      {!isCollapsed && (
        <div className="animate-in fade-in duration-150 divide-y divide-slate-100">
